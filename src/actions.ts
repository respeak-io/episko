...
// And for the OSC title scrub. The loop is the point: `Sess.title` is the *cleaned*
// string, so without re-deriving it from `rawTitle` an idle or ended pane would sit
// there wearing the old rule, which reads as the switch not working.
export function setTitlePrefs(p: TitlePrefs, repaintSettings = true) {
  ...
  renderAll();
  // Off for the keystroke path only: repainting the panel under a live <input> would
  // take the caret with it (see `applyTitleExtra`).
