...
import { isClaude, taskStateText, type Runnable, type Sess } from "./types";
...
function sessionActions(s: Sess): PalItem[] {
  ...
  // Not gated on isClaude — every pane kind has a real directory behind it (a task's
...
function buildPalGroups(raw: string): PalGroup[] {
  ...
  const sessCands: PalItem[] = [...sessions.values()].filter(matchesState).map((s) => {
    ...
    const sub = s.kind === "shell" ? "shell"
      : s.kind === "agent" ? `agent · ${s.phase === "ended" ? "exited" : "running"}`
