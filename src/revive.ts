...
export function reviveStep(
  ...
  // `Sess.agents` holds an agent from its `SubagentStart` until its `SubagentStop`, and
  // agents killed by the same outage that killed the parent turn never send that Stop —
  // so `liveCount` stays high with nothing behind it. Tracking them by id rather than as
  // a counter does not rescue a guard here: the orphan clock only starts when a NEWER
  // fan-out inherits them (`startFanout` stamps `orphanedAt`), and a turn the API killed
  // launches no successor to do the inheriting, so they are believed for the full
  // `FANOUT_DEAD_MS` hour. Worse, the `Workflow` call that owned them died with the turn,
  // so there is no live tool call for a continue to collide with. A guard would therefore
  // stand down for that hour in precisely the scenario this feature exists for — a fleet
  // and its parent going down together — and the hour it cost would be the hour it was
  // supposed to save.
  //
