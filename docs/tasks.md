...
- **The header is a block (`.rgroup`) rather than a `.srow`.** `.rgrow` must stay `position: relative` (its absolutely-positioned ⟳ and ✕ gone static each claim a grid cell and double the header); background is `--surface`, never `--lift` (a white veil reads as nothing on a light theme).
...
Surfaces: `▶ Run` (picker: pinned, then frecency-ranked recent, then by source), a Tasks group in ⌘K, the task inspector (re-run / pin / stop / send output to a session), the run group's header (⟳ the whole stack, ✕ close it). Successful non-background runs auto-dismiss after 20s unless focused; failures persist and raise attention.
