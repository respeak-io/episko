...
- **What can't run says so**: `blocked: Some(reason)` renders greyed, never dropped, because a missing row reads as "Episko didn't find my task". VS Code tasks needing an editor (`${file}`, `${lineNumber}`) or an unsupported `type` are blocked. Supported vars: `workspaceFolder(Basename)`, `cwd`, `userHome`, `pathSeparator`, `env:X`. `${input:X}` is left intact: the frontend prompts (`openInputPrompt`) and substitutes (`applyInputs`) just before launch; `just` params without defaults prompt the same way. **A failed introspector is a blocked row, never an empty list** (`IntrospectFail`), because silence is unfalsifiable. `NoProgram` names the *PATH* rather than saying "not installed" (installed-but-not-visible is the commoner case).

## Run groups: one chain, one row

A chain launches one pane per step (an exit code is a phase; one PTY cannot yield four). `launchWithDeps` mints `run.groupId` **per launch, never per task** (two runs of the same task are two rows to compare); every step inherits it, including the chain's own pane; nesting inherits via `opts.groupId ?? crypto.randomUUID()`, so outermost wins. `run.groupRoot` rides along the same way (`opts.groupRoot ?? r.id`): the chain root can be a compound task with no pane of its own, so once every step is a pane its **id** is only recoverable if it was stored — `groupLabel` names it but cannot launch it.
...
- **The header is a block (`.rgroup`) rather than a `.srow`.** `.rgrow` must stay `position: relative` (its absolutely-positioned ⟳ and ✕ gone static each claim a grid cell and double the header); background is `--surface`, never `--lift` (a 5% white veil is a highlight, not a ground).
- **Two re-runs, and the difference is the point.** A step's own ⟳ (task inspector) reruns *that step alone* through `launchTask` — no dependencies, which is what you want for a `playwright install` that flaked after a two-second `uv sync`. The header's ⟳ (`rerunRunGroup`) stops every member and starts the **whole stack** from `groupRoot` through `runRunnable`, the same attended door as the picker, so an `${input:…}` still asks and a cycle still refuses. A half-failed chain has no other clean start: the failed step's own ⟳ skips the setup it depended on. Both keep the fold — a step re-run inherits its group, and only takes the stage itself while that mosaic is *not* up (`launchTask` stages the group, not the member).
...
- **The mosaic is shared with a session's split shells** (⌘T, docs/sessions.md): `stageGroup` may hold a session id instead of a `groupId`, `inStageGroup` answers membership for both, and `paintStage` lays either out. `groupMembers`, the header and the two ⟳ stay run-only.
- **Closing one tile stays in the mosaic** (`nextInGroup`; `nextAfterClose` answers the *sidebar's* question and would hand the stage elsewhere and untile). Closing a tile must `refit()`, because the grid reflowed while `#terminals` didn't resize, so no ResizeObserver fires.
- **Closing a group asks first if anything still runs** (`closeRunGroup`); a finished chain closes instantly.
- **`setActive` leaves the tiled view** (keeping it read as the click doing nothing), so `openRunGroup` passes `setActive(id, keepGroup = true)`; clicking a *tile* is `focusInGroup`, which moves `activeId` and keeps the layout.
...
- **`compound` and `blocked` are different states**: nothing is missing, and its `detail` names what it runs.
- **`launchWithDeps` returns `{ok, id}`**, two independent facts: a compound *succeeds* while launching no pane; reading that absence as failure would stop a nested compound satisfying its parent.
- **A background dependency is satisfied once it starts, never awaited**, because a dev server never exits, so awaiting hung the chain (VS Code behaves the same). Non-background dependencies are awaited.
- **A compound can't be a run-on-stop rule** (`stopRuleBlocked`): no pane, and `forSession` is cleared for dependencies.
- **`default_for` is separate from `group`**; `runDefaultTask` takes the marked default, else an unambiguous single group member, else **opens the picker**, because silently running the first build-ish task is how you deploy when you meant compile.
- **Shortcuts are bindings now** (`keys.ts`, `matchAction`), and matching is exact, so a shifted chord no longer depends on sitting above its unshifted twin in a chain. `buildTask`/`testTask` default to ⌘⇧B/⌘⇧T and can be rebound or switched off in Settings › Keys.
...
`spawn_task` is the third PTY entry point: `TaskSpec { exec, cwd, env }`, deliberately **un-instrumented** (no settings file, no telemetry, no cost, pid not in `owned_pids`). `Exec::Shell` runs through a login shell, `Exec::Argv` through `argv_command`. The `Exec` wire format is pinned by a round-trip test, since the frontend hands a discovered `exec` straight back to `spawn_task`.

## PATH: a login shell only gets you part way
...
- **Harvested PATH first, fallbacks after**, so nvm's node wins.
...
- **A failure goes back to the session that caused it** (`run.forSession`); the handoff is withheld if that session ended or is external, and typed without a trailing newline, so the human presses Enter.
...
- **Memoise the whole outcome**: `exitWaiters` holds one resolver per session id, and two dependents calling `waitForExit` on the same pane would clobber each other and hang a branch forever.
- **`findDepCycle` runs first, over the whole graph, before any pane starts**, as the memo requires (a branch awaiting a shared task can wait on a branch waiting on it). Pure and tested, including not mistaking a diamond for a cycle.

Dependencies are named by *label*, run in parallel unless `dependsOrder: "sequence"` (parallel is VS Code's own default, surprising as that is), and a failed dependency stops the chain. `waitForExit` resolves from the `pty-exit` listener *before* its early return, and `closeSession` resolves `-1`, so a chain can never deadlock on a vanished pane.

## Inputs

**An input is a second verb rather than a toll on the first**: *Run* goes through `prefillInputs` (last value from `cc-task-inputs` → definition default → empty if `optional`), prompting only when an input has no answer anywhere; changing values is its own button (`⋯` / ⌥⏎ / *⋯ Parameters*). Every attended surface routes through `runRunnable` (picker, ⌘K, the build chord, which toasts only when something actually launched) or shares `resolveRunInputs` (re-run, whose launch semantics differ: it closes the old pane first); run-on-stop is unattended and uses bare `prefillInputs`, never prompting. The row's tooltip shows the command *as prefilled*, since a value reused silently is fine only while visible.

**`optional` exists because `just` has two variadics and they are opposites**: `*name` is complete without a value while `+name` is not, and reading both as required put a prompt in front of every `*args` run. VS Code inputs are never optional. It is also the one thing `stopRuleBlocked` softens on: an input that answers itself is not a prompt.
...
`openTaskManager` (⌘K): pin / hide / create / edit / delete / **override**. `.episko/` is the only place Episko writes in a user's repo (see App-wide rules in CLAUDE.md); editing a discovered task writes an `[override."<id>"]` into `tasks.toml`, **never** a mutation of `.vscode/tasks.json`. Writes go through `toml_edit` so hand-written comments/order survive (tested). Creating the file the first time asks, because a new committable file is a real side effect.

Overrides: `apply_overrides` patches *after* dedupe (keys off final ids); an override whose target vanished becomes a **blocked row** (`override:<id>`) rather than a silent no-op. `save_task_override` writes `background` unconditionally (an override's job includes turning it *off*). Overriding `run` re-derives inputs (`redetect_inputs`). Reverting removes the key, and the whole table when last. The panel reads `list_task_overrides` (the file, not the cache).

Smaller affordances: **package-runner override** (`cc-task-runner`, applied *after* discovery by swapping `exec.program` in `applyRunner`, so the cache needn't know); **remembered inputs** (never a password; see `i.password`); **↗ Reveal source** (`reveal_path`, which guards `..` escape; `run.root` resolves the relative `sourceFile`); **⟳ Rescan** (`rescan_runnables`, the escape hatch for files an introspector imports itself, which the stamp can't see).

Discovery is **memoised in Rust** (`discover_cached`), keyed `(root, trusted)`, invalidated by a *stamp*: the `(mtime, len)` of every file a provider reads, a missing file included. It is not a watcher; ~20 `metadata()` calls answer it. **A new provider file must be added to `source_files()`** or its tasks go stale behind the cache.

Surfaces: `▶ Run` (picker: pinned, then frecency-ranked recent, then by source), a Tasks group in ⌘K, the task inspector (re-run / pin / stop / send output to a session), the run group's header (⟳ the whole stack, ✕ close it). **Every surface that asks "which project is on stage?" reads `paneDir` (./types), never the pane's `workdir`**: a run's workdir is its *command's* cwd, routinely a subfolder a task declared, so ▶ Run opened from a run's own pane discovered there and reported an empty project. `run.root` is the folder discovery ran in and the only thing that answers it, which is why the re-run and the run-on-stop paths pass `discoveredIn` as well — a pane that forgets its root breaks the picker one launch later. Successful non-background runs auto-dismiss after 20s unless focused; failures persist and raise attention.

**A recent row can be forgotten** (hover ✕, or ⇧⌫ on the selection): `forgetFrec` *deletes* `task:<id>` from `cc-frecency` rather than flagging it hidden, since the score is the memory — so there is no second list to keep in sync, and running the task again is the undo. Three consequences worth knowing: the row **moves rather than vanishes** (it drops back into its source group, which is why the toast names that file — a row that only moves reads as a misfire), the ⌘K palette's own Recent group loses the same score (one table, deliberately), and `pinnedIds` is untouched, so the ✕ is never a way to un-pin. The chord is *not* a binding: it lives inside the picker, on a group that only exists with an empty search box, so it can never shadow one.
