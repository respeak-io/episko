...
- **A Claude pane has no scrollback at all, and that is the whole story.** The REPL takes the **alternate screen** — `\x1b[?1049h\x1b[2J\x1b[H` in its first hundred bytes, then `?1000h/?1002h/?1003h/?1006h` to grab the mouse, and no `?1049l` until it exits (probed against the real CLI, not read off a doc). xterm's alternate buffer holds exactly one screen: `baseY` is always 0, `viewportY` never moves, `scrollToLine` is a no-op and `registerMarker` returns a row the next frame paints over. That is why the first cut of this feature jumped to the prompt bar, and why the second — which matched the text — lit up whatever line happened to be showing: the questions were never in the buffer to begin with. The debug line said so at every click (`view 0 → 0`), which is the argument for logging the viewport either side of a jump.
- **So the jump is asked of the TUI, in the language it is already listening in.** It keeps the conversation itself and scrolls on a wheel event, so `wheel()` writes SGR 1006 mouse reports straight to the PTY — the same bytes xterm sends when you turn the wheel over the pane — and the search runs again on the redrawn screen. Four rules make that safe and finite. **Bottom first**, unless the question is on screen already: the view may be parked where the last jump left it, and a hunt that only walks up would never reach a question below that — but a question already showing moves nothing, since yanking to the bottom would undo the reading you are in. **A step is shorter than a screen**: what a notch is worth is the REPL's business, so the first step *measures* it (`screenShift` — how far the rows moved between two readings) and the rest are paced to two thirds of a screen, which is around five times the fixed four-notch crawl this started as. An unreadable shift keeps the opening step, which is only slow, never wrong. The wait between steps is the **redraw itself** (`onWriteParsed` plus a settle, with a timeout for a wheel the REPL ignored) rather than a delay long enough to cover the worst one, and the whole hunt is bounded by time (6s) as well as by steps, so a long conversation gets as far as it can rather than hanging on a click. **A wheel that stops changing the screen is the top of the conversation** (`screenShift` reads 0), which ends a hunt in a young session after one step instead of a hundred; the same reading is what tells the opening snap-to-the-bottom that it is already there, instead of firing a fixed burst of hundreds of notches at the REPL. And a hunt that found nothing **scrolls back to the bottom**, because failing to find your question must not also lose your place. One hunt at a time (two would scroll each other), and a slow one says so after ~0.6s rather than looking dead.
- **A mouse byte is not typing.** This is the only place Episko writes to a live session that is not one of the two documented Enter presses, and it stays outside that rule on purpose: no REPL can read a mouse report as a prompt, and it is exactly what the user's own wheel would have sent. Nothing is ever typed to find a question.
