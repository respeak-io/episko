...
## A session row's own menu

Right-click a row under a project — a live pane, a shelved one or an external one — and `openSessionMenu` (./projmenu) opens the same `#ctxMenu` the project header and the ⑃ cluster header use, carrying that row's verbs. The dashboard's session rows emit `data-sel` too, so they answer the same way.

- **One mode for three kinds.** They answer the same question and differ only in how much of the session is ours: a live pane (all of it), a shelved one (a row and a transcript, so *Resume* and *Take off the shelf*), an external one (neither, so it can only be jumped to). Nothing on an external row stops it — Episko did not start it and `kill_session` cannot reach it.
- **`.srow` is matched before `[data-key]`**, the rule `data-wt` already follows one level up: a shelved or external row carries its *project's* key, so the general lookup would answer a row that has its own menu with the project's.
- **The head is the row's own glyph and label.** `rowGlyph`/`rowLabel`/`pastLabel` are exported from ./sidebarview for it, because the glyph vocabulary already lives in two tables that must move together and a third spelling would drift — and a menu must look like it belongs to the thing you clicked.
- **A row that cannot apply is dropped; one whose absence would read as a bug is greyed with why.** ⇩ follows `canShelve`, the same predicate the header's own button reads. *Switch branch…* needs a branch, and greys while anything in that checkout is mid-turn (`busyIn`, shared with the worktree menu's switch row). *Review working set…* is an agent's only, since only an agent's folder is measured by the dirty sweep, and it reads the app-wide `dirtyByFolder` rather than `Sess.git`, which is only fresh on stage — "not read yet" is not "nothing uncommitted".
- **The branch pop hangs off the sidebar row, not off the menu.** `openSessionBranchPop` asks git first, so the menu is long closed by the time anything is placed; `rowAnchor` re-finds the row by its `data-*` if a repaint replaced the node meanwhile.
- **A task pane carries its own verbs first** — ⟳ *Run again*, ⋯ *Run again with…*, ■ *Stop*, ↗ *Reveal source* and the ▶ Run pin, all of them ./taskrun's and ./tasks', all of them already on the inspector's task card. ⟳ **replaces** the pane rather than adding one and re-runs *this step only*, so its sub says which; ■ keeps the pane and its output where ✕ takes both, and the two subs say so rather than leaving you to find out. A definition that has gone from the last discovery greys the row with *rescan with ▶ Run* instead of toasting after the click. The failed-task handoff stays in the card: it needs a target session picked, which is a submenu rather than a row.
- **Every act is a verb that already exists elsewhere** — ⌘T's shell (`openShellFor`, so `splitBeside` still decides where it lands), the inspector's working-set card, branch chip and task card, the drift card's follow/move, the header's ⇩ and ✕, the shelf's ⟲. The menu adds reach, never a second implementation to keep in step.
...
- **The stage has one owner**: `activeId` and `mirror` (`{kind:"ext"|"past"|"dash"|"fleet"}`) are mutually exclusive; timer-driven inspector repaints must bail on `mirror`, and not merely on the external case; `stageGroup` is a modifier rather than another owner. **`takeStage(show)` in `dom.ts` is the only code that may touch `#extPane`/`#dashPane`/`#fleetPane` and the `stage-*` grid classes**, and **`"home"` is the fleet screen** rather than a splash: `dom.ts` imports nothing, so main.ts hands it `openFleet` through `setStageHome` and every exit from the stage goes through it. `closeExternalView` no longer takes the stage at all — `setActive` takes it for a pane, `leaveMirror` sends it home — because a closer that both clears the pointer and picks the next screen had two callers wanting each half. Openers hiding rivals by hand shipped two-of-four complete: both panes are `position:absolute; inset:0` with **no z-index**, so DOM order decides and a missed hide puts the mirror *behind* an opaque pane, reading as "the click only changed the colours". The dashboard and the fleet each hide the inspector column outright (`.app.stage-dash`, `.app.stage-fleet`); there is no longer a collapsed rail. Add a stage kind by extending `Stage`, never by poking `hidden` at a call site.
...
A resumed pane starts blind: its questions are in the provider's transcript, not in the hooks it is about to receive. So `seedOutline` (./panes, both spawners) does one `readProviderAsked` read at launch and `seedPrompts` (./outline) puts the user turns in front of anything the pane has already recorded — everything the transcript holds predates it. **That read is the questions alone, and from the whole file.** The mirror's reader (`read_transcript`) answers out of the last 512KB, which a day of tool traffic fills many times over: on a real 4.5MB transcript the newest of its four questions sat 3.5MB above that window, so a pane resumed at lunchtime restored none of them and its outline said *one* — the question it had just been asked. A tail is the right read for a mirror, which shows how a conversation ended, and the wrong one for an index of it, so `read_transcript_asked` streams the lot and keeps the user turns (`transcript_msg` is the line parser both share). It runs **once**: a second read would list the lot twice, and no dedupe can tell that from a question genuinely asked twice. `isEnvelope` guards **both** paths, not just this one: Claude Code fires `UserPromptSubmit` for the turns it submits on your behalf — a background task's `<task-notification>`, a `!` line's `<bash-input>`/`<bash-stdout>`, a slash command's echo, the `[Image: source: …]` sidecar a paste files beside a question — so a pane that ran six background tasks listed six of them as questions. The match is on the START of a message alone, and `[Image #1] why is it red?` is a question while `[Image: source: …]` is not.
...
- **One path per buffer kind, and neither renderer is required.** `scrollToPrompt` branches on `buffer.active.type`. A normal buffer — the classic renderer, a shell, a task, any agent that prints — is the scrollback path below: find the line, `scrollToLine`, mark it. The alternate screen is the hunt: the conversation is the TUI's, so the jump is asked of it **with its own keys** — `PageUp`/`PageDown` to page, `Ctrl+Home`/`Ctrl+End` for the two ends, straight from its key table (context `Scroll`) and each probed against the real CLI — with the text search re-run on every redrawn screen. Keys rather than the wheel, which was the first driver: in this TUI a notch is `scroll:lineUp`, **one line**, so a wheel hunt had to measure what a notch was worth and still crawled, while a page is deterministic and never longer than a screen, so a hit cannot be stepped over. Four rules make the hunt safe and finite. **The nearer end first**, unless the question is on screen already, and *which* end is `huntFromTop`'s (./outline): the list is only what this pane recorded, so it places a question in the conversation only when it starts where the conversation does — a fresh pane always, a resumed one once the seed has restored what came before — and anything else is hunted from the live end. Where it does place one, the **clock** picks the end rather than the list position, since rows pile up with time and not with questions. Reading it off the list alone is what shipped: a resumed pane listing a single live question read as *the first of one*, so every click paged down from the top of a fourteen-hour conversation and gave up six seconds later on a question that was 66 pages up from the live end. A question already showing moves nothing, since jumping to an end would undo the reading you are in. **A page that changes nothing is the far end** (`screenShift` reads 0), which ends a hunt in a young session after one page — and it reads a move in **either** direction, since a page forward moves content *up* and a one-way read let the rows that never move (separators, composer, footer: four to six, against a threshold of four) outvote it, so every hunt from the top ended after one PageDown unless the question was already on the first screen (that shipped, 0.25.0). But *a write is not a redraw*: the footer repaints on its own clock, so "no change" is believed only after the full `FRAME_MS`, or the hunt ends on step one (that shipped, for an afternoon). Nor is *half* a redraw one, so the settle restarts on every chunk and a still screen is re-read after a longer `CONFIRM_MS` before it counts as the end — the first page after an end-jump took 273ms to arrive against the real CLI, and a hunt that reads one slow page as the far end never leaves page one. **A time budget as well as a page count**, so a long conversation gets as far as it can rather than hanging on a click: pages redraw in 30-270ms against the real CLI, so the 15s budget is the bound that matters and `HUNT_PAGES` is only a stop. It is that long because 66 pages was a question nine minutes old, and a click that fails is worse than one that takes a moment and says so — the toast lands at 0.7s, and the next click on any row calls the hunt off. And a hunt that found nothing **returns to the live end**, because failing to find your question must not also lose your place — and it says which of the two failures it was, since a hunt that reached the far end has read the whole conversation and can say the question is not in it, while one the budget stopped can only say it is further back than a click reaches. One hunt at a time (two would page each other), and a slow one says so after ~0.7s rather than looking dead. The log line names the pane, the direction, the page count and why the hunt stopped — never the text.
