...
| `SOUND_GAP_MS` | 350ms | **different events** in one burst, since a fleet moving together is one moment |
...
is the thing you actually needed to hear, whatever it repeats. That is what the
...
`launched` are not, and the reason is blunt: **a set of alerts that fires on every
...
second reading here would be a second copy of that decision, and the copy that drifts. So
...
baseline rather than a crossing**, or every launch above 50% would ring at the one moment the
...
listener. Without that, the failure surfaces as nothing at all: a permission that
...
gate, because you are looking straight at the window, which is the one moment `away` would
