...
- **Other agents, in the same panes.** Which agent a session runs is a setting, not a question you answer every time: pick it in Settings › Sessions, override it per project from the project's own menu, and `⌘N` obeys. Episko offers the coding-agent CLIs you actually have installed — Codex, OpenCode, Gemini, Cursor, Copilot, Grok, Droid, Amp, Qwen, Kimi, Kiro, Antigravity, Devin, Cline, Pi and more, twenty-one in all — each in a real pane in the right checkout, with the project tree, `⌘K` and the working-set view around it. They run *uninstrumented*, and every launch surface says so rather than faking it: phase, cost and context come from hooks only Claude Code reads.
