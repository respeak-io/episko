...
| `env.rs` | which `.env` each of a checkout's environments is pointed at: the `[env]` table, target patterns whose directory segment may wildcard, presets resolved beside their own target, the content match that decides the active preset, and the swap |
...
| `envs.ts` | what `env.rs`'s facts mean: which preset each environment is on, the ordered regex rules that name and colour it (first match wins, always case-insensitive, a broken one skipped rather than fatal), what one chip says for a checkout that has several, and where a saved preset lands |
...
- **Which `.env` an environment is on is decided by reading it, never by remembering it.** The
  active preset is whichever one the target's *content* matches, so the chip stays right when
  you edit the file elsewhere, switch worktrees, or an agent rewrites it mid-turn — and the
  only state worth a confirm is content that matches **no** preset, which exists nowhere else.
  **A checkout has as many environments as it has targets**: one `.env` at the root is the
  one-element case, a monorepo has one per package, and presets resolve *beside their own
  target* so no package offers another's. A target pattern's directory segment may hold a `*`,
  and expanding one is the only thing here that reads a directory to find its children — capped,
  and never into a dot-dir or a build output. `git ls-files` cannot be the source, since `.env*`
  is gitignored almost everywhere. The backend answers facts and `envs.ts` owns the regex rules
  (./health's division, and a user-typed pattern belongs in the engine that cannot take the app
  down). Four surfaces draw the chip — stage header, status bar, dashboard card, and a sidebar
  mark for **danger only** — and all four open one picker, which with several environments
  speaks for the worst and counts the rest. A flagged checkout raises `#envBadge`, its **own**
  badge beside the telemetry and servers ones and never `#attnBadge`: that one is
  `attnPending`, "a session is waiting on you", and nothing here is waiting. **The rules belong
  to the project**, so ./envdlg writes them into `.episko/episko.toml` and Settings holds only
  the fallback — there is no project settings page in this app, and adding one here would have
  made a global page do project work. Ships **off**. Nothing watches the filesystem; the tick
  plus an agent's own file writes are the whole answer (docs/environments.md).
...
- **`docs/environments.md`**: the `.env` switcher. The file is the state (content decides the active preset, so nothing is remembered); a checkout has one environment per target, and presets resolve beside their own; the backend answers facts and the frontend owns the regex; the rules are the project's and a dialog writes them there; the danger badge is its own, never the reactor.
