...
| `fleet.ts` | the all-projects dashboard's rules: one card per project, the three sorts, and the three states of a checkout nobody has swept |
...
- **The stage has one owner**: `activeId` and the `mirror` pointer (`{kind:"ext"|"past"|"dash"|"fleet"}`) are mutually exclusive, and `takeStage(show)` in `dom.ts` is the only code that may touch `#extPane`/`#dashPane`/`#fleetPane`/`#empty` and the `stage-*` grid classes. Add a stage kind by extending `Stage`, never by poking `hidden` at a call site — and note that **`tsc` cannot flag a widened `mirror` union**: `setMirror` still accepts every call and every `mirror?.kind === "x"` site still compiles, so the thirteen read sites have to be walked by hand.
