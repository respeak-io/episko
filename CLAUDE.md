...
## Backend (`src-tauri/src/`): eighteen modules

`main.rs` only calls `episko_lib::run()`. `lib.rs` is the **bootstrap**; the backend logic is the seventeen modules under it. Dependencies point downward, `platform.rs` at the bottom. Rust tests are in-file `#[cfg(test)] mod tests`, next to their subject.
...
| `usage.rs` | transcripts (incl. History's whole-machine scan) + the token ledger; everything read out of `~/.claude`, plus the one account fact that tree does not hold: the per-model plan limits, asked of the CLI (`claude_usage_limits`) |
...
| `github.rs` | `gh`: issues/PRs, one thread read in full (body + comments) behind the queue's ⤢, the claim writes, closing, the committed keep list, the merged-PR evidence behind the broom's force; `gh()`/`classify()` are `pub(crate)` so `deps.rs` shares one account-and-token path rather than a second copy |
...
| `sync.rs` | the sync server connection (docs/sync.md): one WebSocket held from Rust with its backoff, pairing, the token sealed per OS (DPAPI, the keychain, a 0600 file), the persisted cursor, and `sync-event`. Decides nothing about what syncs |
...
`AppState` holds the telemetry `port`, `sync` (the sync connection's control), `sessions: HashMap<session_id, Session>` (each = PTY master + writer + child killer), `agent_runtimes` (provider control-plane child + control channel), `owned_pids` (Claude external-registry filtering; see docs/sessions.md), `io_samples`, `io_retired`, the held-open Claude `pending` permission requests, and `caffeinate`.
...
## Frontend (`src/`, `index.html`, `src/styles.css`): 105 modules

**No framework, and no longer one file.** 105 modules; `main.ts` is **bootstrap only**. State lives in a `sessions: Map<session_id, Sess>` (owned by `state.ts`) plus module-level variables; **every mutation ends by calling `renderAll()`**, which re-renders the sidebar, mini-rail, inspector, header, footer, attention badge, and tray from scratch. There is no diffing, so follow this render-everything pattern rather than mutating DOM directly. **`renderAll()` is coalesced**: a call only marks the pass due, and one flush per animation frame paints whatever state every event in that frame left behind, so a telemetry burst from N sessions costs a single paint. The rAF is paired with a 250ms `setTimeout` fallback, and that is not belt-and-braces: rAF never fires while the window is hidden, and the tray this pass repaints is exactly the surface being read then. The 🐞 console counts paints beside received events (`paints` in the stats line), so the batching is checkable while the app runs.
...
| `emoji.ts` | the emoji picker over the whole Unicode set: what a query matches and in what order, the suggested row, and the rows the virtualised grid paints (`rowWindow` — the two spacers, since only a screenful of ~1,900 glyphs is ever in the DOM). Its data is `emojidata.ts`, generated from Unicode's `emoji-test.txt` by `scripts/gen-emoji.mjs`; skin tones and hair are left out of it, and the panel's field takes them anyway |
...
| `rl.ts` | account-wide rate limits: merging readings, burn rate, the window forecast, and the per-model weekly windows the statusLine never carries (a level, never a forecast) |
...
| `grouping.ts` | what the sidebar shows and in what order; `urgencyRank`, `needsYou`/`attnPending`/`syncAttn`, `nextAfterClose`, `dormantBusy`, and the stage's groups: the run-group fold (`foldRunGroups`, `groupPhase`, `nextInGroup`) and a session's split shells (`stageKeyOf`, `inStageGroup`, `splitShells`, `splitAnchorFor`) |
...
| `issue.ts` | one thread read in full: what its line says, and markdown enough for a body and its comments — code spans out first, links http(s) only, an image drawn rather than fetched |
...
| `sync.ts` | what may leave the machine and how it comes back: the `cc-` key allowlist (`SYNC_KEYS`, every key classified or the suite fails), last-writer-wins, usage partitioned by device, the limits merge, and when sync counts as down (docs/sync.md) |
| `roster.ts` | the seven path-keyed stores on the wire, spoken in project ids: `rosterWire`, `applyWire` (one entry onto this machine's checkout, or held until one exists), and `mergeWire`, which carries every entry for a project not cloned here so no machine ever sends the absence of what it cannot see |
...
**Markup-only views**, untested by design: `usageview`, `inspectorview`, `sidebarview`, `issueview` (the thread reader: one issue or pull request, its body and its comments, in the `"issue"` overlay), `depsview` (the Dependencies card, its three-tab overlay and its dispatch sheet — split out of `dashview` rather than grown inside it, which is why `dispatch.test.ts` now reads BOTH for `data-dash*` and fails if a third view file starts emitting them), `patchview` (the diff viewer's files, hunks and index — split out of `diffview` when it grew two line layouts, and where `hunkHtml` moved from `inspectorview`, whose only caller it never was; it also owns the one status-letter table and the one dirty-file row every host that *lists* a working set draws, since two hand-kept copies of that table each carried a comment claiming to be shared with the other), `landedview` (the *Landed* section: the lite commit graph's rows, its ref chips and its fold row — the newest rows of one page of `graph.ts`'s own layout, with none of `graphview`'s paging, scrolling or message overlay), `fleetview` (the all-projects dashboard's markup; it emits only `data-fl*`, which is what keeps it out of the `data-dash*` contract and inside its own), `syncview` (Settings › Sync: the pairing form, the connection's state, what this device sent), `footerview` (the engine picker, the shortcut sheet and `popGoHtml`, the quick-open icon every status-bar popover carries — extracted from `footer.ts`, which owns those elements, so Settings' previews of the popovers can paint them with the real renderer).

**DOM-owning / render**, untested by design: `sidebar`, `footer`, `tray`, `inspector`, `confirm` (every yes/no question in the app), `menu` (the popover menu: a title, groups of items and a pick, anchored to the control it belongs to — a separator is a rule *between two groups* and so can never be drawn beside nothing, which a row list carrying its own `null` separators could and did), `callsheet` (the tool-call window: the dialog, its list/detail split and the two independent `innerHTML` guards that let you select text in it), `debug`, `worktree` (the new-session dialog and the worktree removal flows, the biggest single module), `settings`, `usagedlg` (the Usage & spend window: a report rather than a setting, so its own dialog, and the target of the money and limits popovers' quick opens), `taskui`, `palui`, `projmenu`, `caffeinate`, `signoff` (the top bar's sign-off sheet: shelve the whole fleet at once, docs/sessions.md), `diffview` (the working-set review overlay: the dialog, its index rail, the scroll spy and which line layout is current), `graphview` (the paged commit-graph panel), `mirror`, `historyui`, `update`, `serversui` (the header's running-server pill, its popover and the poll behind it), `explorer` (⌘P, the project explorer), `fleetui` (the all-projects pane: its two whole-fleet reads, its own `#fleetPane` listener and the `FleetHost` seam), `tourui` (the veil, the card and the chapter picker), `syncui` (the sync badge beside the telemetry one, and the status bar's sync segment), `chime` (the only file that touches Web Audio, a live browser resource, so a test would only assert against its own mock).

**Behaviour**, IPC and DOM all the way down, so untested too, and therefore the thinnest ice in the app: `panes` (the four spawners + a pane's lifecycle), `terminal` (the xterm plumbing), `taskrun` (run on stop), `actions` (the app-level verbs), `icons` (the per-project glyph store), `rlprobe` (one throttled `claude_usage_limits` call, made when a surface that shows per-model limits opens), `synclink` (sync's driver: the one `Storage.prototype.setItem` hook that sees every write, the outbox, and applying what other machines sent).

Four rules keep that graph honest. **There are no import cycles across the 105 modules; re-run a cycle check after any change that adds an import.**
...
- **⌘T opens a shell BESIDE the session on stage, and a split shell is never `activeId`.** The
  stage's tiling (`stageGroup`, `paintStage` in ./panes) is one mechanism for run groups and
  splits: a split is keyed by the anchor session's own id, the anchor keeps the left column at
  full height and its shells stack on the right (`--split-rows`). A shell is an attachment: a
  tile click or its nested sidebar row moves the ring and the keyboard (`focusTile`), never
...
- **Event wiring**: `listen("pty-output" | "pty-exit" | "telemetry" | "permission" | "agent-event" | "sync-event" | "tray-select")` at the bottom of `main.ts`. Claude telemetry routes by stable launch id; provider events carry `sessionId` + `provider` and are rejected if either does not match the pane.
...
- **A premium model's weekly window reaches no telemetry, so it is ASKED for, and it is a
  level.** The statusLine carries `five_hour` and `seven_day` and nothing else, so Fable's
  separate allowance — routinely the window that binds first on an account that runs one —
  is invisible to every hook. ./rlprobe asks the CLI over its control protocol
  (`get_usage`, no model turn and no tokens) when the limits popover or *Usage & spend*
  opens, never on a timer. Three rules travel with it: the probe **closes stdin rather than
  killing**, or it leaves a stale `~/.claude/sessions/<pid>.json` behind; **absent
  `model_scoped` is not an empty one** (absent means nothing was learned and the last
  reading stands, `[]` is the account having no such window), and the **label is the
  server's**, so a new model bucket needs no release — reading `seven_day_opus` by name
  would have shipped a meter that is null for exactly the accounts that need it. One
  reading has no slope, so these surfaces drop the burn rate, projection and time-to-cap
  rather than printing the level back with a `~`. `docs/architecture.md`.
...
- **Every ahead/behind figure is as old as the last fetch, so Episko fetches for the pane
  you are looking at.** `upstream_state` reads `refs/remotes` and never the network, which is
  why a session opened on a branch a colleague had moved used to say *in sync* and mean it.
  ./autofetch owns the rule and `tickAutoFetch` runs it: the **active pane's checkout only**,
  on arrival and on a 20s tick that mostly answers "not due" — a behind count nobody is
  reading is not worth a round trip, and one fetch serves every worktree of a repo, so the
  record is keyed by repo (`fetchedByRepo`, on `colorKey`) and not by checkout. It calls the
  same `git_action(op:"fetch")` the buttons do, quietly: **no toast, no terminal handoff**,
  since nobody asked for this one — the failure goes to the log and to the sync chip's
...
- **The stage has one owner**: `activeId` and the `mirror` pointer (`{kind:"ext"|"past"|"dash"|"fleet"}`) are mutually exclusive, and `takeStage(show)` in `dom.ts` is the only code that may touch `#extPane`/`#dashPane`/`#fleetPane` and the `stage-*` grid classes. **`takeStage("home")` is the fleet screen** (`setStageHome`, wired in main.ts): every exit from the stage — the last session closed, a dashboard or a mirror stepped out of, the boot with nothing restored — lands on the all-projects dashboard, so *no sessions running* is not a screen this app has. Add a stage kind by extending `Stage`, never by poking `hidden` at a call site — and note that **`tsc` cannot flag a widened `mirror` union**: `setMirror` still accepts every call and every `mirror?.kind === "x"` site still compiles, so the thirteen read sites have to be walked by hand.
...
- **A permission the OS asks about is Episko's problem even when it is an agent that
  reached.** macOS blames the *responsible* process, so a session reading another app's data
  raises a dialog in our name. Settings › Privacy (macOS only) explains it, probes the one
  grant that can be given in advance, opens the pane and undoes a denial — and offers full
  disk access without ever recommending it, because every agent inherits it (docs/macos-access.md).
...
- **`docs/sync.md`**: the self-hosted sync server. The server accelerates git and `localStorage` and is never the authority; the key list is an allowlist; anything that changes what runs or what is permitted stays local; usage is partitioned by device, never merged.
