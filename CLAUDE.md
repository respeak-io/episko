...
Test coverage is **unit-only — there is no end-to-end harness**, but it is no longer thin: **468 vitest + cargo (96 on macOS, 93 on Windows — the platform tests are `cfg`-gated)**, both run in CI on both OSes.
...
`main.rs` only calls `episko_lib::run()`. `lib.rs` is **bootstrap, not the backend**: 502 lines out of ~9,900 (the counts below are whole files, in-file `#[cfg(test)] mod tests` included — that is most of `telemetry.rs`). Dependencies point downward, `platform.rs` at the bottom.
