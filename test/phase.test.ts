...
// Records every invoke, so clearPending's release (or its absence) can be asserted.
...
describe("applyHook — the lifecycle state machine", () => {
  ...
  // Claude Code's idle Notification fires a minute later whether the turn finished or
  // died; it must not relabel a StopFailure as "your turn" (docs/architecture.md).
  ...
  describe("the activity timeline", () => {
    ...
    // The next three carry no `tool_use_id`, so pairing falls back to the tool name:
    // the fallback's spec, not the preferred path.
    ...
    it("drops a Post whose id matches nothing rather than closing another call's row", () => {
      // The name match is for a payload with NO id, never a second chance for an id that
      // missed: falling back here would staple this output onto another call's row.
      const s = sess();
  ...
  // Pre and Post pair by `tool_use_id`, never by tool name: two calls of one tool overlap
  // routinely under parallel subagents, and output on the wrong row is a lie the card
  // states in full.
  describe("what a tool call ran and what it returned", () => {
    it("pairs Pre with Post by tool_use_id, whatever order the answers arrive in", () => {
      ...
      // t1 answers while t2 is the newest open Bash; a name match would put its output on t2.
    ...
    // A PostToolUseFailure carries no `tool_response`; the reason is a plain string in `error`.
    ...
    // Capped at capture, not in the view: a Read response is a whole file, and the ring holds twelve.
    ...
    // Keyed, not indexed: the ring shifts under a live session and an index would move an open row.
  ...
  describe("subagent depth suppresses the phase flips", () => {
    // A subagent's tool calls arrive on the parent's hooks; letting them drive the phase
    // would flap the glyph.
    ...
    it("applies the same guard to a turn that just finished", () => {
      // The hooks are concurrent curls, so a turn's last PostToolUse can land just after
      // its Stop; it must not take the pane back to "working".
    ...
    it("but lets a tool call past that window drive the phase again", () => {
      // `done` must not suppress tool hooks forever: a turn with no UserPromptSubmit of
      // its own (the model carrying on, a queued prompt) would run behind a green tick.
      const s = sess({ phase: "done" });
  ...
  // A fan-out is the one case where the turn ending says nothing about the work ending;
  // the cockpit's readout is assembled from hooks alone (see Fanout in types.ts).
...
`;
    ...
    describe("parseWorkflowMeta", () => {
      ...
      it("stops at the meta literal, so an agent prompt can't supply a name or a phase", () => {
        // Below the literal is arbitrary JavaScript full of `name:` and `title:`; an
        // unbounded match would label the run "Bob".
      ...
      it("survives a script with no meta at all", () => {
        // Never throws: the counts come from the hooks, and an unnamed fleet is still worth showing.
    ...
    it("names the fleet from the Workflow call, before a single agent has started", () => {
      // Claude Code's own run-state file is written when the run finishes; the hook lands 2s in.
      const s = sess({ phase: "working" });
    ...
    it("clears the fleet on SessionStart and SessionEnd", () => {
      // /clear, /compact and a resume mint a new Claude session; a SubagentStop we will
      // never see would pin the count above zero forever.
    ...
    it("keeps the fleet across a prompt typed while it runs", () => {
      // Clearing on UserPromptSubmit would drop the run's name mid-flight.
      const s = sess();
      hook(s, "PreToolUse", { tool_name: "Workflow", tool_input: { script: WF } });
      hook(s, "SubagentStart");
      hook(s, "UserPromptSubmit");
    ...
    it("names a resumed run from its script path, since a resume carries no script", () => {
      // A resume passes `scriptPath` + `resumeFromRunId` and no `script`; the persisted
      // file is named `<meta.name>-<runId>.js`.
      const s = sess();
    ...
    it("keeps the description and phases across a resume the pane launched inline", () => {
      // The filename recovers only the name; the record from the inline parse carries the
      // rest and wins when the names agree.
      const s = sess();
      hook(s, "PreToolUse", { tool_name: "Workflow", tool_input: { script: WF } });
      hook(s, "PreToolUse", { tool_name: "Workflow", tool_input: {
    ...
    describe("what the cockpit reads off it", () => {
      // The turn is over, the fleet is not.
      ...
      it("counts a second workflow's agents rather than showing 14 of 2", () => {
        // A second fan-out restarts the counters while the first fleet is still up, so
        // `started` alone lags the live count.
        const s = fleet();
        hook(s, "PreToolUse", { tool_name: "Workflow", tool_input: { script: WF } });
        setPhase(s, "done");
      ...
      it("yields to a permission and to a broken turn", () => {
        // Both outrank a fleet: Claude blocked on you now, or a turn that will not come back.
      ...
      it("stands down once the fleet has been quiet for the grace window", () => {
        // The grace window covers the lulls between a workflow's stages, where the live
        // count is legitimately 0.
        const s = fleet();
      ...
      it("keeps a long-running agent alive past the window", () => {
        // An agent can run eighteen minutes without one event; a recency rule would drop
        // the biggest runs first.
        const s = fleet();
        vi.setSystemTime(NOW_MS + FANOUT_GRACE_MS * 10);
        expect(bgWaiting(s)).toBe(true);
      });
      it("writes the fleet off once the silence outlasts what any real run has shown", () => {
        // A SubagentStop can genuinely never come (an interrupted run, a dropped POST);
        // past FANOUT_DEAD_MS the silence outvotes the counter.
      ...
      it("zeroes the leaked count on the next event, so no later fleet inherits it", () => {
        // total is `done + running`: a ghost left in the set would put a fresh burst at 0/2.
        const s = fleet();
        vi.setSystemTime(NOW_MS + FANOUT_DEAD_MS + 1000);
        hook(s, "UserPromptSubmit");
        expect(liveCount(s)).toBe(0);
      ...
      it("starts a new fleet for a burst after the old one stood down", () => {
        // Resuming the retired record would show 12 of 14 done for two agents that just launched.
        const s = fleet();
    ...
    // Both hooks carry `agent_id`, so an agent is retired by name rather than by a counter
    // that can only drift up.
    it("ages an inherited agent faster than a live fleet, which is the whole point", () => {
      // A real event re-stamps `lastAt` and revives a fleet; an orphan has no run left to
      // report it, so the same hour would be a ghost's life support.
    ...
    describe("agents by id", () => {
      ...
      it("ignores a Start replayed under an id already up", () => {
        // A curl the CLI retried, or a POST that landed twice.
        const s = sess();
      ...
      it("falls back to the oldest agent when the payload carries no id", () => {
        // An older CLI, or a hook shape we mis-read: a synthetic id in, oldest-first out.
        const s = sess();
        hook(s, "SubagentStart"); hook(s, "SubagentStart");
        expect(liveCount(s)).toBe(2);
        hook(s, "SubagentStop");
    ...
    // `startFanout` restarts the counters while the live set carries over whole; see
    // `Agent.orphanedAt` and the "34 / 36" bug in docs/architecture.md.
    describe("a fan-out that inherits the last one's agents", () => {
      // `orphans` left behind by the interrupted burst; `total` agents in the replacement run, all landed.
      ...
      it("keeps counting them while they could still be real", () => {
        // Not dropped on sight: launching a second workflow over one still finishing is ordinary.
      ...
      it("does not credit their Stops to the run that inherited them", () => {
        // `done` is this run's agents and nobody else's; an old fleet's straggler is not progress.
        const s = inherit(2, 34);
      ...
      it("writes them off on their own clock, not the live run's", () => {
        // The inheriting run keeps re-stamping `lastAt`, so the hour that guards a live
        // fleet never elapses; an orphan ages from the moment it was inherited.
        const s = inherit(2, 34);
      ...
      it("hands the session back to you once they expire", () => {
        // `bgWaiting` suppresses the reactor badge, the tray title and the palette's
        // "Needs you" group.
        const s = inherit(2, 34);
        expect(statusKey(s)).toBe("background");
      ...
      it("sweeps the expired ones out of the set on the next event", () => {
        // The read side applies the window; the sweep is what stops a later fleet
        // inheriting the ghosts in turn.
        const s = inherit(2, 34);
        vi.setSystemTime(NOW_MS + ORPHAN_DEAD_MS + 1000);
        hook(s, "UserPromptSubmit");
      ...
      it("still counts a genuinely overlapping fleet", () => {
        // A second workflow launched while the first is really running; its agents are retired by name.
        const s = inherit(3, 2);
  ...
  // UserPromptSubmit fires the instant Enter is pressed, even mid-turn (the message is
  // queued), so the outstanding turn's Stop lands AFTER the new prompt.
  describe("a prompt queued behind a running turn", () => {
    ...
    it("excuses exactly one Stop, however many prompts were queued", () => {
      // Queued messages do not get a Stop each: one turn usually answers several, so a
      // counter would be left holding a phantom prompt.
      const s = sess();
      hook(s, "UserPromptSubmit");
    ...
    it("holds the run-on-stop rule back until the last one is answered", () => {
      // A run at the first Stop would verify work the agent is about to change under it.
      const seen: string[] = [];
    ...
    it("clears the flag on a failed turn, so it cannot excuse the next one", () => {
      // The phase stays `error` (./revive watches it); only the flag is spent.
    ...
    it("lets the 60s idle nudge repair a flag that was never spent", () => {
      // A queued prompt cancelled with Esc fires no Stop; a REPL idle for a minute is
      // proof nothing is queued.
      const s = sess({ phase: "working", queuedPrompt: true });
      hook(s, "Notification", { notification_type: "idle_prompt" });
      expect(s).toMatchObject({ phase: "done", queuedPrompt: false });
    });
    it("drops the queue with the REPL that held it", () => {
      // /clear, /compact and a resume each mint a fresh conversation; nothing queued survives it.
      for (const ev of ["SessionStart", "SessionEnd"]) {
        const s = sess({ phase: "working", queuedPrompt: true });
        hook(s, ev);
    ...
    it("ends a turn green when nothing was queued behind it", () => {
      // A pane adopted mid-turn, or one whose UserPromptSubmit was lost, must still earn its tick.
      const s = sess({ phase: "working" });
      hook(s, "Stop");
      expect(s).toMatchObject({ phase: "done", queuedPrompt: false });
    });
...
describe("applyStatusline — the meters, and the proof a session is alive", () => {
  ...
  it("leaves every field it wasn't told about untouched", () => {
    // Fields come and go across Claude Code releases; a missing one must not blank a live meter.
  ...
  describe("cost", () => {
    ...
    it("does not re-book the running total when a resume replaces the pane", () => {
      // Move session, restore and a History reopen launch a new `Sess` (`cost: null`) for a
      // conversation Claude resumes with its total intact (docs/sessions.md).
      ...
      // Both panes stay named: the money moved once, the attribution did not.
  ...
  describe("rate limits", () => {
    ...
    it("keeps the peak when a lagging session reports a staler number", () => {
      // Each session's statusLine carries the account numbers only as fresh as it last refreshed them.
...
describe("riskLevel — the badge on a pending permission", () => {
  ...
  // Regression guard: `rm\s+-[rf]\b` never matched the combined "rm -rf", so the form
  // people actually type rated a mere "review".
