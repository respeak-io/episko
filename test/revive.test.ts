...
function sess(o: Partial<Sess> = {}): Sess {
  return {
    id: "s1", kind: "claude", external: false, phase: "error", attention: null,
    apiErr: { kind: "overloaded", detail: "", at: NOW }, revive: null,
    subagents: 0, fanout: null,
    ...o,
  } as Sess;
}
...
describe("reviveStep — the things it must never do", () => {
  ...
  it("is NOT held off by a fan-out counter left high by the same outage", () => {
    ...
    // parent never send `SubagentStop`, so the counter stays high with nothing behind it.
    // Standing down for it would sleep through the exact scenario this feature is for.
    const s = sess({ subagents: 7, fanout: { name: "review", lastAt: NOW, started: 7, done: 0 } as never });
